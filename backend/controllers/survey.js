...
function custIndPartial(Cust_Code) {
  ...
  BEGIN
      ...
      ,D.IS_FATCA AS fatca,convert(varchar, D.FATCA_DATE, 112) AS fatcaDeclarationDate
  ...
  try {
    ...
    // Incase has Email
      ...
      <div class="a">
        ...
        <p class='txt-Indent'>
    เพื่อให้การปฏิบัติงานเป็นไปตามประกาศกำหนดของสำนักงานป้องกันและปราบปรามการฟอกเงิน และสำนักงานคณะกรรมการกำกับหลักทรัพย์และตลาดหลักทรัพย์ เรื่อง การทบทวนข้อมูลลูกค้าและการทำแบบประเมินความเสี่ยงของผู้ลงทุน
        </p>
        <p class='txt-Indent'>
       ...
       */
        transporter.sendMail(mailOptions, (error, info) => {
          ...
          res.status(200).json({ code:"000",message: 'Send mail successful!' });
        });
        // Incase No Email
      }else{
